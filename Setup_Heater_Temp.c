...
void Increase_Heater_Temp()
{
	...
	// then check if it is greater than 120. If so, limit to 120
        In_Heater_Temp_Setup++;
    	Update_Setup_Heater_Screen();                       // Update screen with latest info
        while(In_Heater_Temp_Setup > 120)                        // then check if it is less than 50. If so, limit to 50
        {
            In_Heater_Temp_Setup = 120;
...
void Decrease_Heater_Temp()          
{
        ...
        In_Heater_Temp_Setup--;                                 // add code to decrease setup_heater_set_temp by 1
        Update_Setup_Heater_Screen();                           // Update screen with latest info
        
        while(In_Heater_Temp_Setup < 50)                        // then check if it is less than 50. If so, limit to 50
        {
            In_Heater_Temp_Setup = 50;
            Update_Setup_Heater_Screen(); 
        }
}
